...
class ClassicalDataset:
    ...
    def __init__(
        ...
        train_df = pd.read_csv(train_csv, encoding='utf-8', encoding_errors='replace')
        test_df = pd.read_csv(test_csv, encoding='utf-8', encoding_errors='replace')
        ...
        # --- Fit char TF-IDF on train only (optional) ---
        ...
        if use_char_ngrams:
            ...
            self._char_tfidf.fit(train_df[text_col].fillna(''))
    ...
    def _build(self, df: pd.DataFrame):
        ...
        # Char TF-IDF (sparse, optional)
        if self.use_char_ngrams and self._char_tfidf is not None:
            X_char = self._char_tfidf.transform(texts)
    ...
    def feature_names(self) -> list:
        ...
        if self.use_char_ngrams and self._char_tfidf is not None:
            names += [f"char_{n}" for n in self._char_tfidf.get_feature_names_out().tolist()]
        if self.use_vader:
